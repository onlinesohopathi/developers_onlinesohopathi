function loadallprofiless(quesid,ids)
{
...
//      notifications=0;  	
	...
	var uploadtoDb="v1/index.php/uploadblogcomment";
	...
	data.append('content',answer);
	
    data.append('anonymous',anonymous);
	
	data.append('userpic',fbpic);
	
    data.append('blogid',question_id);
...
function parsingoneBlog(filt){
   ...
   var Obj;
    //var onequestion=localStorage.getItem("new_tab_question");
    var onequestion=filt;
	
    //alert("onequestion".concat(onequestion));
    
   var data=new FormData();
   data.append('oneblog',onequestion);
